...
class LaporanIzinDivisiController extends Controller
{
...
public function index(Request $request)
    {
        ...
        $baseQuery = LaporanIzinDivisi::query()
        ...
        $laporanizindivisis = (clone $baseQuery)->orderBy('tanggal', 'desc')->paginate($perPage);
        ...
        $data = $allSickReports->pluck('total_izin')->all();
        ...
        return view('hrga.laporanizindivisi', compact('laporanizindivisis', 'chartData'));
    }
...
public function store(Request $request)
    {
        try {
            ...
            $exists = LaporanIzinDivisi::where('tanggal', $validatedData['tanggal'])
            ...
            LaporanIzinDivisi::create($validatedData);

            return redirect()->route('laporanizindivisi.index')->with('success', 'Data Berhasil Ditambahkan');
        } catch (\Exception $e) {
            // Logging untuk debug
            ...
            return redirect()->route('laporanizindivisi.index')->with('error', 'Terjadi Kesalahan:' . $e->getMessage());
        }
        try {
            $validatedData = $request->validate([
                'tanggal' => 'required|date',
            ...
            $exists = LaporanIzinDivisi::where('divisi', $validatedData['divisi'])->exists();
...
public function update(Request $request, LaporanIzinDivisi $laporanizindivisi)
    {
        try {
            // Validasi input
            $validatedData = $request->validate([
            ...
            // Cek kombinasi unik date dan divisi
            $exists = LaporanIzinDivisi::where('divisi', $validatedData['divisi'])
                ->where('id_sakit', '!=', $laporanizindivisi->id_sakit)->exists();
            ...
            // Update data
            $laporanizindivisi->update($validatedData);

            // Redirect dengan pesan sukses
            return redirect()->route('laporanizindivisi.index')->with('success', 'Data berhasil diperbarui.');
        } catch (\Exception $e) {
...
public function destroy(LaporanIzinDivisi $laporanizindivisi)
    {
        try {
            $laporanizindivisi->delete();
            return redirect()->route('laporanizindivisi.index')->with('success', 'Data Berhasil Dihapus');
        } catch (\Exception $e) {
