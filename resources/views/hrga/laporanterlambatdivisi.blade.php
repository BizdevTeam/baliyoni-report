    ...
    <head>
        ...
        <title>Late Arrival Report</title>
    ...
    <body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
        <div class="wrapper">
            ...
            <div id="admincontent" class="mt-14 content-wrapper p-4 bg-gray-50 duration-300">
                <h1 class="flex text-4xl font-bold text-red-600 justify-center mt-4">Late Arrival Report</h1>
                <div class="flex items-center justify-end transition-all duration-500 mt-8 mb-4">
                    <!-- Search -->
                    <form method="GET" action="{{ route('laporanterlambatdivisi.index') }}" class="flex items-center gap-2">
                    ...
                    <div class="mx-auto bg-white p-6 rounded-lg shadow">
                        <div class="overflow-x-auto bg-white shadow-md">
                            <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                                ...
                                <tbody>
                                    @forelse ($laporanterlambatdivisis as $laporanterlambatdivisi)
                                    <tr class="hover:bg-gray-100">
                                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanterlambatdivisi->tanggal_formatted }}</td>
                                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanterlambatdivisi->nama }}</td>
                                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanterlambatdivisi->divisi }}</td>
                                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanterlambatdivisi->total_terlambat }}</td>
                                        <td class="border border-gray-300 py-2 text-center flex justify-center gap-2">
                                            <button class="bg-transparent text-blue-600 px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporanterlambatdivisi->id_laporan_terlambat_divisi }}"><i class="fa fa-pen"></i></button>
                                            <form method="POST" action="{{ route('laporanterlambatdivisi.destroy', $laporanterlambatdivisi->id_laporan_terlambat_divisi) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <div class="flex justify-between items-center mt-4">
                            ...
                            <div>
                                {{ $laporanterlambatdivisis->withQueryString()->links('pagination::tailwind') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="formChart" class="visible">
                    <div class="flex flex-col mx-auto bg-white p-6 mt-4 rounded-lg shadow-xl border border-grey-500 chart-group">
                        <div class="mb-4 flex justify-between items-center">
                            <h1 class="text-2xl font-bold text-red-600 font-montserrat mx-auto">Late Arrival Report Chart</h1>
                ...
                <!-- Modals -->
                @foreach ($laporanterlambatdivisis as $laporanterlambatdivisi)
                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $laporanterlambatdivisi->id_laporan_terlambat_divisi }}">
                    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                        <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                        <form method="POST" action="{{ route('laporanterlambatdivisi.update', $laporanterlambatdivisi->id_laporan_terlambat_divisi) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="space-y-4">
                                <div>
                                    <label for="tanggal" class="block text-sm font-medium">Date</label>
                                    <input type="date" name="tanggal" class="w-full p-2 border rounded" value="{{ $laporanterlambatdivisi->tanggal }}" required>
                                </div>
                                <div>
                                    <label for="nama" class="block text-sm font-medium">Nama Karyawan</label>
                                    <input type="text" name="nama" class="w-full p-2 border rounded" value="{{ $laporanterlambatdivisi->nama }}" required>
                                </div>
                                <div>
                                    ...
                                    <select name="divisi" class="w-full p-2 border rounded" required>
                                        <option value="Marketing" {{ $laporanterlambatdivisi->divisi == 'Marketing' ? 'selected' : '' }}>Marketing</option>
                                        <option value="Procurement" {{ $laporanterlambatdivisi->divisi == 'Procurement' ? 'selected' : '' }}>Procurement</option>
                                        <option value="Accounting" {{ $laporanterlambatdivisi->divisi == 'Accounting' ? 'selected' : '' }}>Accounting</option>
                                        <option value="IT" {{ $laporanterlambatdivisi->divisi == 'IT' ? 'selected' : '' }}>IT</option>
                                        <option value="HRGA" {{ $laporanterlambatdivisi->divisi == 'HRGA' ? 'selected' : '' }}>HRGA</option>
                                        <option value="Support" {{ $laporanterlambatdivisi->divisi == 'Support' ? 'selected' : '' }}>Support</option>
                                        <option value="SPI" {{ $laporanterlambatdivisi->divisi == 'SPI' ? 'selected' : '' }}>SPI</option>
                                    </select>
                                </div>
                                <div>
                                    <label for="total_terlambat" class="block text-sm font-medium">Sick Leave Total</label>
                                    <input type="number" name="total_terlambat" class="w-full p-2 border rounded" value="{{ $laporanterlambatdivisi->total_terlambat }}" required>
                                </div>
                            </div>
                ...
                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="addEventModal">
                    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                        ...
                        <form method="POST" action="{{ route('laporanterlambatdivisi.store') }}" enctype="multipart/form-data">
                            @csrf
                            <div class="space-y-4">
                                <div>
                                    <label for="tanggal" class="block text-sm font-medium">Date</label>
    ...
    renderChartsFromTable();
        ...
        document.getElementById('exportPdfButton')?.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch('/hrga/laporanterlambatdivisi/export-pdf', {
                    method: 'POST',
                    headers: {
