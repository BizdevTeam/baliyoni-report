...
<body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div id="admincontent" class="mt-14 content-wrapper ml-64 p-4 bg-white duration-300">
            <h1 class="flex text-4xl font-bold text-red-600 justify-center mt-4">Laporan Sakit</h1>
        ...
        <div class="overflow-x-auto bg-white shadow-md">
            <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                ...
                <tbody>
                    @foreach ($laporansakits as $laporansakit)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporansakit->tanggal_formatted }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporansakit->nama }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporansakit->total_sakit }}</td>
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-transparent text-red-600 px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporansakit->id_sakit }}">
                                    <i class="fa fa-pen"></i>
                                </button>
                                <!-- Delete Form -->
                                <form method="POST" action="{{ route('laporansakit.destroy', $laporansakit->id_sakit) }}">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $laporansakit->id_sakit }}">
                            <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                ...
                                <form method="POST" action="{{ route('laporansakit.update', $laporansakit->id_sakit) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="space-y-4">
                                        <div>
                                            <label for="tanggal" class="block text-sm font-medium">Tanggal</label>
                                            <input type="date" name="tanggal" class="w-full p-2 border rounded" value="{{ $laporansakit->tanggal }}" required>
                                        </div>
                                        <div>
        ...
        <div class="m-4">
            {{ $laporansakits->links('pagination::tailwind') }}
        </div>
    </div>
</div>
...
<div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="addEventModal">
    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
        ...
        <form method="POST" action="{{ route('laporansakit.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="tanggal" class="block text-sm font-medium">Tanggal</label>
...
<script>
    ...
    const items = Array.from(document.querySelectorAll('#data-table tr')).map(row => {
        ...
        return {
            ...
            total_sakit: cells[2]?.innerText.trim() || '',
        };
    });

    const tableContent = items
        .filter(item => item.tanggal && item.nama && item.total_sakit)
        .map(item => `
            <tr>
                ...
                <td style="border: 1px solid #000; padding: 8px; text-align: center;">${item.total_sakit}</td>
            </tr>
        `).join('');
    ...
    try {
        const response = await fetch('/hrga/laporansakit/export-pdf', {
            method: 'POST',
            headers: {
