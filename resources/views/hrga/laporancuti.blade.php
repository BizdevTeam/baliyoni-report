...
<body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan HRD (CUTI)</h1>
                ...
                <form id="modal-form" class="space-y-4" method="POST">
                    ...
                    <div id="nama-container">
                        <div class="nama-item flex items-center space-x-2 mb-2">
                            ...
                            <input type="text" name="total_cuti[]" class="w-full border-gray-300 rounded p-2"
                                placeholder="Total Cuti" required>
                            <button type="button"
        ...
        <table class="w-full table-auto border-collapse border border-gray-300 mt-6">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th class="border border-gray-300 px-4 py-2">Total Cuti</th>
    ...
    <script>
        document.getElementById('add-nama').addEventListener('click', () => {
            ...
            cutiInput.type = 'text';
            cutiInput.name = 'total_cuti[]';
            cutiInput.className = 'w-full border-gray-300 rounded p-2';
            cutiInput.placeholder = 'Total Cuti';
            ...
            newnamaItem.appendChild(cutiInput);
        ...
        async function fetchData() {
            try {
                const response = await fetch('/hrga/laporancuti/data');
        ...
        modalForm.addEventListener('submit', async (e) => {
            ...
            const cutiList = [...document.querySelectorAll('input[name="total_cuti[]"]')].map(
                input => parseFloat(input.value.trim()));

            if (!bulanTahun || namaList.some(p => !p) || cutiList.some(isNaN)) {
            ...
            const url = editMode ? `/hrga/laporancuti/update/${editId}` :
                '/hrga/laporancuti/store';
        ...
        async function deleteData(id) {
            ...
            try {
                const response = await fetch(`/hrga/laporancuti/destroy/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    },
        ...
        async function updateData(filter = '') {
            const url = filter ? `/hrga/laporancuti/data?bulan_tahun=${filter}` :
                '/hrga/laporancuti/data';
        ...
        function updateTable(items) {
            ...
            if (items.length === 0) {
                ...
                <tr class="border-b">
                    ...
                    <td class="border px-4 py-2">${item.total_cuti.toLocaleString()}</td>
        ...
        function updateChart(items) {
                ...
                const dataValues = items.map(item => item.total_cuti);
                ...
                window.myChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                callbacks: {
                                    label: context => `Total Cuti ${context.raw.toLocaleString()} Kali`,
                                },
                            },
                        },
                        scales: {
        ...
        function editData(id, data) {
            ...
            // Pastikan `total_cuti` adalah array
            parsedData.total_cuti = Array.isArray(parsedData.total_cuti) ?
                parsedData.total_cuti :
                (typeof parsedData.total_cuti === 'string' ? parsedData.total_CUTI.split(',').map(Number) : []);
            ...
            parsedData.nama.forEach((nama, index) => {
                ...
                cutiInput.className = 'w-full border-gray-300 rounded p-2';
                cutiInput.placeholder = 'Total Cuti';
                cutiInput.value = parsedData.total_cuti[index] || ''; // Set nilai pendapatan
                cutiInput.required = true;
