            ...
            <div id="formContainer" class="visible">
                <div class="mx-auto bg-white p-6 rounded-lg shadow">
                    ...
                    <div class="overflow-x-auto bg-white shadow-md">
                        <table class="table-auto w-full border-collapse border border-gray-300">
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-100">
                                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $ijasagambar->tanggal_formatted }}</td>
                                    <td class="border border-gray-300 px-4 py-2 text-center">
                                        <div class="relative hover:scale-[1.5] transition-transform duration-300">
                                            @if ($ijasagambar->gambar)
                                            <img src="{{ asset('images/hrga/ijasagambar/' . $ijasagambar->gambar) }}" alt="Eror Image" class=" shadow-md hover:shadow-xl rounded-md transition-shadow duration-300 h-16 mx-auto object-cover cursor-pointer" onclick="openModal('{{ asset('images/hrga/ijasagambar/' . $ijasagambar->gambar) }}')">
                                            @else
                                            <img src="{{ asset('images/no_image.png') }}" alt="Default Image" class="h-16 mx-auto">
                                            @endif
                                        </div>
                                    </td>
                                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $ijasagambar->keterangan }}</td>
                                    <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                        <!-- Edit Button -->
                                        <button class="bg-transparent text-red-600 px-3 py-2 rounded" data-modal-target="#editEventModal{{ $ijasagambar->id_ijasa_gambar }}">
                                            <i class="fa fa-pen"></i>
                                        </button>
                                        <!-- Delete Form -->
                                        <form method="POST" action="{{ route('ijasagambar.destroy', $ijasagambar->id_ijasa_gambar) }}">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $ijasagambar->id_ijasa_gambar }}">
                        ...
                        <div class="m-4">
                            {{ $ijasagambars->links('pagination::tailwind') }}
                        </div>
                    </div>
                </div>
            </div>
