    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">iJASA</h1>
                <!-- Button Tambah Data -->
                    ...
                    <div class="bg-white p-6 rounded shadow w-full max-w-md">
                        ...
                        <form id="modal-form" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="time" id="modal-jam" name="jam"
                                    class="w-full border-gray-300 rounded p-2" required>
                            </div>
                            <div>
                            ...
                            <div>
                                <label for="modal-troubleshooting"
                                    class="block text-sm font-medium">Troubleshooting</label>
                            ...
                            <div>
                                <label for="modal-resolve_tanggal"
                                    class="block text-sm font-medium text-red-500">Tanggal
                                    Resolusi</label>
                ...
                <!-- Table -->
                <table class="table-auto border-collapse border border-gray-300 mt-6 items-center text-center ">
                    <thead class="bg-gray-200">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2 text-center">Tanggal Resolusi</th>
                            <th class="border border-gray-300 px-4 py-2 text-center">Jam Resolusi</th>
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    async function updateData(filter = '') {
                        ...
                        try {
                            ...
                            table.innerHTML = ''; // Bersihkan isi tabel sebelum menambahkan data baru

                            result.data.forEach((item) => {
                                const row = document.createElement('tr');
                                row.innerHTML = `
                           <td class="border px-4 py-2 text-left whitespace-nowrap">${item.tanggal}</td>
                            <td class="border px-4 py-2 text-left whitespace-nowrap">${item.jam}</td>
                            <td class="border px-4 py-2 text-left">${item.permasalahan}</td>
                            <td class="border px-4 py-2 text-left">${item.impact}</td>
                            <td class="border px-4 py-2 text-left">${item.troubleshooting}</td>
                            <td class="border px-4 py-2 text-left whitespace-nowrap">${item.resolve_tanggal}</td>
                            <td class="border px-4 py-2 text-left whitespace-nowrap">${item.resolve_jam}</td>
                            <td class="border px-4 py-2 text-left">
                                <div class="flex flex-col items-center space-y-2">
                                    <!-- Tombol Edit -->
                                    <button onclick="editData(${item.id}, decodeURIComponent('${encodeURIComponent(JSON.stringify(item))}'))"
                                        class="flex items-center justify-center bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded text-sm">
