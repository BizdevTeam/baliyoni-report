...
<body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan HRD (TERLAMBAT)</h1>
                ...
                <form id="modal-form" class="space-y-4" method="POST">
                    ...
                    <div id="nama-container">
                        <div class="nama-item flex items-center space-x-2 mb-2">
                            ...
                            <input type="text" name="total_terlambat[]" class="w-full border-gray-300 rounded p-2"
                                placeholder="Total Terlambat" required>
                            <button type="button"
    ...
    <script>
        document.getElementById('add-nama').addEventListener('click', () => {
            ...
            const terlambatInput = document.createElement('input');
            terlambatInput.type = 'text';
            terlambatInput.name = 'total_terlambat[]';
            terlambatInput.className = 'w-full border-gray-300 rounded p-2';
            terlambatInput.placeholder = 'Total Terlambat';
            terlambatInput.required = true;
            ...
            newnamaItem.appendChild(terlambatInput);
        ...
        async function fetchData() {
            try {
                const response = await fetch('/hrga/laporanterlambat/data');
        ...
        modalForm.addEventListener('submit', async (e) => {
            ...
            const terlambatList = [...document.querySelectorAll('input[name="total_terlambat[]"]')].map(
                input => parseFloat(input.value.trim()));

            if (!bulanTahun || namaList.some(p => !p) || terlambatList.some(isNaN)) {
        ...
        function updateTable(items) {
            ...
            if (items.length === 0) {
                ...
                <tr class="border-b">
                    ...
                    <td class="border px-4 py-2">${item.total_terlambat.toLocaleString()}</td>
                ...
                window.myChart = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                callbacks: {
                                    label: context => `Total Terlambat ${context.raw.toLocaleString()} Kali`,
                                },
                            },
                        },
                        scales: {
        ...
        function editData(id, data) {
            ...
            // Pastikan `total_terlambat` adalah array
            parsedData.total_terlambat = Array.isArray(parsedData.total_terlambat) ?
                parsedData.total_terlambat :
                (typeof parsedData.total_terlambat === 'string' ? parsedData.total_terlambat.split(',').map(Number) : []);
            ...
            parsedData.nama.forEach((nama, index) => {
                ...
                terlambatInput.className = 'w-full border-gray-300 rounded p-2';
                terlambatInput.placeholder = 'Total Terlambat';
                terlambatInput.value = parsedData.total_terlambat[index] || ''; // Set nilai pendapatan
                terlambatInput.required = true;
