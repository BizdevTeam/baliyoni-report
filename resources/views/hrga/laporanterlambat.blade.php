    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan Cuti</h1>
        <!-- Action Buttons -->
        <div class="flex items-center mb-4 gap-2">
            <form method="GET" action="{{ route('laporantelat.index') }}" class="flex items-center gap-2">
        ...
        <div class="overflow-x-auto bg-white shadow-md">
            <table class="table-auto w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    @foreach ($laporantelats as $laporantelat)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporantelat->bulan_formatted }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporantelat->nama }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporantelat->total_telat }}</td>
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-red-600 text-white px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporantelat->id_telat }}">
                                    <i class="fa fa-pen"></i>
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                <form method="POST" action="{{ route('laporantelat.destroy', $laporantelat->id_telat) }}">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $laporantelat->id_telat }}">
                            <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                                <form method="POST" action="{{ route('laporantelat.update', $laporantelat->id_telat) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="space-y-4">
                                        <div>
                                            <label for="bulan" class="block text-sm font-medium">Bulan</label>
                                            <input type="month" name="bulan" class="w-full p-2 border rounded" value="{{ $laporantelat->bulan }}" required>
                                        </div>
                                        <div>
                                            <label for="nama" class="block text-sm font-medium">Nama Karyawan</label>
                                            <input type="text" name="nama" class="w-full p-2 border rounded" value="{{ $laporantelat->nama }}" required>
                                        </div>
                                        <div>
                                            <label for="total_telat" class="block text-sm font-medium">Total Terlambat</label>
                                            <input type="number" name="total_telat" class="w-full p-2 border rounded" value="{{ $laporantelat->total_telat }}" required>
                                        </div>
                                    </div>
        ...
        <div class="m-4">
            {{ $laporantelats->links('pagination::tailwind') }}
        </div>
        </div>
        </div>
...
<div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="addEventModal">
    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
        ...
        <form method="POST" action="{{ route('laporantelat.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="bulan" class="block text-sm font-medium">Bulan</label>
                ...
                <div>
                    <label for="total_telat" class="block text-sm font-medium">Total Terlambat</label>
                    <input type="text" name="total_telat" class="w-full p-2 border rounded" required>
                </div>
            </div>
            <div class="mt-4 flex justify-end gap-2">
                <button type="button" class="bg-red-600 text-white px-4 py-2 rounded" data-modal-close>Close</button>
