                ...
                <tbody>
                    @foreach ($laporanptboss as $laporanptbos)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanptbos->date_formatted }}</td>
                            ...
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-transparent text-red-600 px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporanptbos->id_ptbos }}">
                                    <i class="fa fa-pen"></i>
                                </button>
                                <!-- Delete Form -->
                                ...
                                <form method="POST" action="{{ route('laporanptbos.update', $laporanptbos->id_ptbos) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="date" name="date" class="w-full p-2 border rounded" value="{{ $laporanptbos->date }}" required>
                                        </div>
                                        <div>
...
async function exportToPDF() {
    ...
    const tableContent = items
        .filter(item => item.date && item.pekerjaan)
        .map(item => `
            <tr>
