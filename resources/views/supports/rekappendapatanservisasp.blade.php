                ...
                <tbody>
                    @foreach ($rekappendapatanservisasps as $rekappendapatanservisasp)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $rekappendapatanservisasp->date_formatted }}</td>
                                ...
                                <form method="POST" action="{{ route('rekappendapatanservisasp.update', $rekappendapatanservisasp->id_rpsasp) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="date" name="date" class="w-full p-2 border rounded" value="{{ $rekappendapatanservisasp->date }}" required>
                                        </div>
                                        <div>
...
<script>
    ...
    const tableContent = items
        .filter(item => item.date && item.pelaksana && item.nilai_pendapatan)
        .map(item => `
            <tr>
