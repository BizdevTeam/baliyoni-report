...
<head>
    ...
    <title>Rekap Piutang Service ASP</title>
    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Rekap Piutang Service ASP</h1>
        <!-- Action Buttons -->
        <div class="flex items-center mb-4 gap-2">
            <form method="GET" action="{{ route('rpiutangsasp.index') }}" class="flex items-center gap-2">
        ...
        <div class="overflow-x-auto bg-white shadow-md">
            <table class="table-auto w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    @foreach ($rpiutangsasps as $rpiutangsasp)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $rpiutangsasp->bulan_formatted }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $rpiutangsasp->pelaksana }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $rpiutangsasp->nilai_formatted }}</td>
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-red-600 text-white px-3 py-2 rounded" data-modal-target="#editEventModal{{ $rpiutangsasp->id_rpiutangsasp }}">
                                    <i class="fa fa-pen"></i>
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                <form method="POST" action="{{ route('rpiutangsasp.destroy', $rpiutangsasp->id_rpiutangsasp) }}">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $rpiutangsasp->id_rpiutangsasp }}">
                            <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                                <form method="POST" action="{{ route('rpiutangsasp.update', $rpiutangsasp->id_rpiutangsasp) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="space-y-4">
                                        <div>
                                            <label for="bulan" class="block text-sm font-medium">Bulan</label>
                                            <input type="month" name="bulan" class="w-full p-2 border rounded" value="{{ $rpiutangsasp->bulan }}" required>
                                        </div>
                                        <div>
                                            ...
                                            <select name="pelaksana" class="w-full p-2 border rounded" required>
                                                <option value="CV. ARI DISTRIBUTION CENTER" {{ $rpiutangsasp->pelaksana == 'CV. ARI DISTRIBUTION CENTER' ? 'selected' : '' }}>CV. ARI DISTRIBUTION CENTER</option>
                                                <option value="CV. BALIYONI COMPUTER" {{ $rpiutangsasp->pelaksana == 'CV. BALIYONI COMPUTER' ? 'selected' : '' }}>CV. BALIYONI COMPUTER</option>
                                                <option value="PT. NABA TECHNOLOGY SOLUTIONS" {{ $rpiutangsasp->pelaksana == 'PT. NABA TECHNOLOGY SOLUTIONS' ? 'selected' : '' }}>PT. NABA TECHNOLOGY SOLUTIONS</option>
                                                <option value="CV. ELKA MANDIRI (50%)-SAMITRA" {{ $rpiutangsasp->pelaksana == 'CV. ELKA MANDIRI (50%)-SAMITRA' ? 'selected' : '' }}>CV. ELKA MANDIRI (50%)-SAMITRA</option>
                                                <option value="CV. ELKA MANDIRI (50%)-DETRAN" {{ $rpiutangsasp->pelaksana == 'CV. ELKA MANDIRI (50%)-DETRAN' ? 'selected' : '' }}>CV. ELKA MANDIRI (50%)-DETRAN</option>
                                            </select>
                                        </div>
                                        <div>
                                            <label for="nilai_pendapatan" class="block text-sm font-medium">Nilai</label>
                                            <input type="number" name="nilai_pendapatan" class="w-full p-2 border rounded" value="{{ $rpiutangsasp->nilai_pendapatan }}" required>
                                        </div>
                                    </div>
        ...
        <div class="m-4">
            {{ $rpiutangsasps->links('pagination::tailwind') }}
        </div>
        </div>
        </div>
...
<div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="addEventModal">
    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
        ...
        <form method="POST" action="{{ route('rpiutangsasp.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="bulan" class="block text-sm font-medium">Bulan</label>
