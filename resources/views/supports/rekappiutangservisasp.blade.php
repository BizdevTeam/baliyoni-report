                ...
                <tbody>
                    @foreach ($rekappiutangservisasps as $rekappiutangservisasp)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $rekappiutangservisasp->bulan_formatted }}</td>
                                ...
                                <form method="POST" action="{{ route('rekappiutangservisasp.update', $rekappiutangservisasp->id_rpiutangsasp) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="month" name="bulan" class="w-full p-2 border rounded" value="{{ $rekappiutangservisasp->bulan }}" required>
                                        </div>
                                        <div>
...
var ctx = document.getElementById('chart').getContext('2d');
    ...
    const tableContent = items
        .filter(item => item.bulan && item.pelaksana && item.nilai_piutang)
        .map(item => `
            <tr>
