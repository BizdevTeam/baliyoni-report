    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan Negosiasi</h1>
        <!-- Action Buttons -->
        <div class="flex items-center mb-4">
            <form method="GET" action="{{ route('laporannegosiasi.index') }}">
            ...
            <table class="table-auto w-full border-collapse border border-gray-300">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-center">Total Negosiasi</th>
                ...
                <tbody>
                    @foreach ($laporannegosiasis as $laporannegosiasi)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporannegosiasi->bulan_formatted }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporannegosiasi->negosiasi_formatted }}</td>
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-red-600 text-white px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporannegosiasi->id_negosiasi }}">
                                    <i class="fa fa-pen"></i>
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                ...
                                <form method="POST" action="{{ route('laporannegosiasi.update', $laporannegosiasi->id_negosiasi) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="month" name="bulan" class="w-full p-2 border rounded" value="{{ $laporannegosiasi->bulan }}" required>
                                        </div>
                                        <div>
        ...
        <div class="m-4">
            {{ $laporannegosiasis->links('pagination::tailwind') }}
        </div>
        </div>
        </div>
