...
<body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
            ...
            <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-center">Nilai Holding</th>
                ...
                <tbody>
                    @foreach ($laporanholdings as $laporanholding)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanholding->date_formatted }}</td>
                            ...
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-red-600 text-white px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporanholding->id }}">
                                    <i class="fa fa-pen"></i>
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                ...
                                <form method="POST" action="{{ route('laporanholding.update', $laporanholding->id) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="date" name="date" class="w-full p-2 border rounded" value="{{ $laporanholding->date }}" required>
                                        </div>
                                        <div>
        ...
        <div class="m-4">
            {{ $laporanholdings->links('pagination::tailwind') }}
        </div>
    </div>
</div>
...
<script>
    ...
    const tableContent = items
        .filter(item => item.date && item.laporanholding && item.nilai)
        .map(item => `
            <tr>
