...
<body class="bg-gray-100 hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div id="admincontent" class="mt-14 content-wrapper ml-64 p-4 bg-white duration-300">
            <h1 class="flex text-4xl font-bold text-red-600 justify-center mt-4">Stock Report</h1>
            ...
            <div class="ai-insight mt-4 p-4 bg-white rounded-lg shadow">
                <h3 class="text-lg font-semibold mb-2">Analisis Stok</h3>
            ...
            <div class="flex items-center justify-end transition-all duration-500 mt-8 mb-4">
                <form method="GET" action="{{ route('laporanstok.index') }}" class="flex items-center gap-2">
                ...
                <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                    <thead class="bg-gray-200">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2 text-center">Total Stock Value</th>
                            <th class="border border-gray-300 px-4 py-2 text-center">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($laporanstoks as $laporanstok)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <button class="text-red-600 bg-transparent px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporanstok->id_stok }}">
                                    <i class="fa fa-pen"></i>
                                </button>
                                <form method="POST" action="{{ route('laporanstok.destroy', $laporanstok->id_stok) }}">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="m-4">
                {{ $laporanstoks->withQueryString()->links('pagination::tailwind') }}
            </div>
    ...
    <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="addEventModal">
        <div class="bg-white w-1/2 p-6 rounded shadow-lg">
            ...
            <form method="POST" action="{{ route('laporanstok.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label for="stok" class="block text-sm font-medium">Total Stock</label>
    ...
    <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $laporanstok->id_stok }}">
        <div class="bg-white w-1/2 p-6 rounded shadow-lg">
            ...
            <form method="POST" action="{{ route('laporanstok.update', $laporanstok->id_stok) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label for="stok" class="block text-sm font-medium">Total Stock</label>
                        <input type="number" name="stok" class="w-full p-2 border rounded" value="{{ $laporanstok->stok }}" required>
                    </div>
                </div>
                <div class="mt-4 flex justify-end gap-2">
                    <button type="button" class="bg-red-600 text-white px-4 py-2 rounded" data-modal-close>Close</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderChart() {
        ...
        tableRows.forEach(row => {
            ...
            if (cells.length > 1) {
                // Kolom 0 adalah Tanggal, Kolom 1 adalah Nilai Stok
        ...
        const chartDataFromTable = {
            labels: labels,
            datasets: [{
                label: 'Total Stock Value', // Label disesuaikan
                data: dataValues,
...
async function exportToPDF() {
    ...
    const tableContent = items.map(item => `
        <tr>
            ...
            <td style="border: 1px solid #000; padding: 8px; text-align: right;">${item.stok}</td>
        </tr>
    `).join('');
