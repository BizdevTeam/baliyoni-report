    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100">
            <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan Stok</h1>
                    <!-- Button Tambah Data -->
        ...
        <div id="modal" class="hidden fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center">
            <div class="bg-white p-6 rounded shadow w-full max-w-md">
                ...
                <form id="modal-form" class="space-y-4">
                    ...
                    <div>
                        <label for="modal-stok" class="block text-sm font-medium">Total Stok
                            (RP)</label>
                        <input type="text" id="modal-stok" name="stok"
...
>>>>>>> 7b312b5ae6bd4eab4284aa255993e815f479f8b5
        ...
        <table class="w-full table-auto border-collapse border border-gray-300 mt-6">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th class="border border-gray-300 px-4 py-2">Total Stok (RP)</th>
    ...
    <script>
        ...
        modalForm.addEventListener('submit', async (e) => {
            ...
            const url = editMode ? `/procurements/laporanstok/update/${editId}` : '/procurements/laporanstok/store';
        ...
        async function deleteData(id) {
            ...
            try {
                const response = await fetch(`/procurements/laporanstok/destroy/${id}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Content-Type': 'application/json',
        ...
        async function updateDataByYear(year) {
            try {
                const response = await fetch(`/procurements/laporanstok/filter?tahun=${year}`, {
                    method: 'GET',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    },
        ...
        async function updateData(filter = '') {
            const url = filter ? `/procurements/laporanstok/filter?tahun=${filter}` : '/procurements/laporanstok/data';
            try {
                const response = await fetch(url);
                const result = await response.json();
        ...
        function updateChart(items) {
            ...
            const dataValues = items.map((item) => item.stok);
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    return `Stok : Rp ${context.raw.toLocaleString()}`;
                                },
                            },
                        },
                    },
