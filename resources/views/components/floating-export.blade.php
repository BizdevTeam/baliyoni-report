<form id="exportForm" action="{{ route('export-laporan-all-new') }}" method="get" target="_blank">
    ...
    <!-- PERBAIKAN: Kelas 'fixed' dipindahkan ke div pembungkus ini -->
    <div class="group fixed bottom-80 right-6 flex items-center">
        
        <!-- Tooltip (Tidak ada perubahan di sini, sudah benar) -->
        ...
        <!-- PERBAIKAN: Kelas 'fixed' dihapus dari tombol ini -->
        <button id="openExportModalBtn" type="button"
                class="w-20 h-20 justify-center rounded-full bg-red-600 font-medium text-white px-4 py-3 hover:shadow-xl transition duration-300 ease-in-out transform hover:scale-105 flex items-center gap-2">
