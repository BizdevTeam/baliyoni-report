    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Laporan PPN</h1>

        <!-- Action Buttons -->
        <div class="flex items-center mb-4 gap-2">
            <form method="GET" action="{{ route('laporanppn.index') }}" class="flex items-center gap-2">
        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg">
            <table class="table-auto w-full border-collapse border border-gray-300">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="border border-gray-300 px-4 py-2 text-center">Files</th>
                ...
                <tbody>
                    @foreach ($laporanppns as $laporanppn)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanppn->bulan_formatted }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                @if ($laporanppn->thumbnail)
                                    <img src="{{ asset('images/accounting/ppn/' . $laporanppn->thumbnail) }}" alt="Default Image" class="h-16 mx-auto">
                                @else
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                @if ($laporanppn->file)
                                <a href="{{ asset('files/accounting/ppn/' . $laporanppn->file) }}"
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporanppn->keterangan }}</td>
                            <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                <!-- Edit Button -->
                                <button class="bg-red-600 text-white px-3 py-2 rounded" data-modal-target="#editEventModal{{ $laporanppn->id_laporanppn }}">
                                    <i class="fa fa-pen"></i>
                                    Edit
                                </button>
                                <!-- Delete Form -->
                                <form method="POST" action="{{ route('laporanppn.destroy', $laporanppn->id_laporanppn) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button class="bg-red-500 text-white px-3 py-2 rounded" onclick="return confirm('Are you sure to delete?')">
                                ...
                                <form method="POST" action="{{ route('laporanppn.update', $laporanppn->id_laporanppn) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <input type="month" name="bulan" class="w-full p-2 border rounded" value="{{ $laporanppn->bulan }}" required>
                                        </div>
                                        <div>
                                            ...
                                            <div class="mt-2">
                                                <img src="{{ asset('images/accounting/ppn/' . $laporanppn->thumbnail) }}" alt="Thumbnail Image" class="h-16">
                                            </div>
                                        </div>
                                        <div>
            ...
            <!-- Pagination -->
            <div class="m-4">
                {{ $laporanppns->links('pagination::tailwind') }}
            </div>
        </div>
