            ...
            <div id="formChart" class="visible">
                <div class="flex flex-col mx-auto bg-white p-6 mt-4 rounded-lg shadow-xl border border-grey-500">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button id="exportPdfButton" class="bg-red-700 text-white px-6 py-3 rounded-lg shadow-md hover:bg-red-600 transition duration-300 ease-in-out">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        /**
         * Renders the bar chart by aggregating package values per website.
         */
        function renderChart() {
            const tableRows = document.querySelectorAll('#data-table tbody tr');
            const websiteData = {}; // Use an object to aggregate data

            // **FIX:** Iterate over table rows to aggregate data by website
            tableRows.forEach(row => {
                const cells = row.querySelectorAll('td');
                if (cells.length > 2) { // Ensure row has enough cells
                    const website = cells[1].innerText.trim();
                    // **FIX:** Get the package value from the correct column (index 2)
                    ...
                    if (!isNaN(numericValue)) {
                        if (websiteData[website]) {
                            websiteData[website] += numericValue; // Add to existing website total
                        } else {
                            websiteData[website] = numericValue; // Initialize new website total
                        }
                    }
                }
            });
            
            const labels = Object.keys(websiteData);
            const dataValues = Object.values(websiteData);
            const backgroundColors = labels.map(() => `rgba(${Math.floor(Math.random() * 200)}, ${Math.floor(Math.random() * 200)}, ${Math.floor(Math.random() * 200)}, 0.7)`);

            const chartData = {
                labels: labels,
                datasets: [{
                    label: 'Total Package Value by Website', // Updated label for clarity
                    data: dataValues,
                    ...
                    afterDatasetsDraw(chart) {
                        ...
                        data.datasets.forEach((dataset, i) => {
                            ...
                            meta.data.forEach((bar, index) => {
                                ...
                                // **FIX:** Display the value with "Package(s)" text
                                ctx.fillText(`${value.toLocaleString('id-ID')} Package(s)`, bar.x, bar.y - 5);
                            });
                        });
                        ctx.restore();
                    }
        ...
        async function exportToPDF() {
            ...
            // **FIX:** Correctly map all three columns from the table
        ...
        /**
         * Changes the number of items per page and reloads.
         */
