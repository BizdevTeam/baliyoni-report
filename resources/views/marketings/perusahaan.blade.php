    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="mt-14 content-wrapper ml-64 p-4 bg-white duration-300">
            ...
            <div class="flex items-center justify-end transition-all duration-500 mt-8 mb-4">
                <!-- Search -->
                <form method="GET" action="{{ route('perusahaan.index') }}" class="flex items-center gap-2">
                        ...
                        <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                        ...
                                        <button class="text-red-500 px-3 py-2 rounded"
                                            data-modal-target="#editEventModal{{ $perusahaan->id }}">
                                            <i class="fa fa-pen"></i>
                                        </button>
                                        <!-- Delete Form -->
                                        <form method="POST"
                                ...
                                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden"
                                    id="editEventModal{{ $perusahaan->id }}">
                                    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                        <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                                        <form method="POST"
                                            action="{{ route('perusahaan.update', $perusahaan->id) }}"
...
async function exportToPDF() {
    ...
    const tableContent = items
        .filter(item => item.bulan && item.nama_perusahaan)
        .map(item => `
            <tr>
    ...
    try {
        const response = await fetch('/marketings/perusahaan/export-pdf', {
            method: 'POST',
            headers: {
