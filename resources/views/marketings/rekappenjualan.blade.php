        ...
        <div id="admincontent" class="mt-14 content-wrapper ml-64 p-4 bg-white duration-300">
            ...
            <div id="formContainer" class="hidden">
                <div class="mx-auto bg-white p-6 rounded-lg shadow">
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                        <!-- Edit Button -->
                                        <button class="bg-transparent text-red-600 px-3 py-2 rounded" data-modal-target="#editEventModal{{ $rekappenjualan->id_rp }}">
                                            <i class="fa fa-pen"></i>
                                        </button>
                                        <!-- Delete Form -->
                                        <form method="POST" action="{{ route('rekappenjualan.destroy', $rekappenjualan->id_rp) }}">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden" id="editEventModal{{ $rekappenjualan->id_rp }}">
                                    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                        <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                                        <form method="POST" action="{{ route('rekappenjualan.update', $rekappenjualan->id_rp) }}" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <div class="space-y-4">
                                                <div>
                                                    <label for="tanggal" class="block text-sm font-medium">Date</label>
                                                    <input type="date" name="tanggal" class="w-full p-2 border rounded" value="{{ $rekappenjualan->tanggal }}" required>
                                                </div>
                                                <div>
                                                    <label for="total_penjualan" class="block text-sm font-medium">Total
                                                        Sales</label>
                                                    <input type="number" name="total_penjualan" class="w-full p-2 border rounded" value="{{ $rekappenjualan->total_penjualan }}" required>
                                                </div>
                                            </div>
                    ...
                    <div class="m-4">
                        {{ $rekappenjualans->withQueryString()->links('pagination::tailwind') }}
                    </div>
                </div>
            </div>
            <div id="formChart" class="visible">
                <div class="flex flex-col mx-auto bg-white p-6 mt-4 rounded-lg shadow-xl border border-grey-500">
                    <h1 class="text-2xl mx-auto font-bold text-red-600 mb-2 font-montserrat text-start">Sales Recap Chart</h1>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderChart() {
            ...
            tableRows.forEach(row => {
                ...
                if (cells.length > 1) {
                    ...
                    // Correctly parse the formatted currency string back to a number
                    const valueString = cells[1].innerText.trim().replace(/[^0-9,]/g, '').replace(',', '.');
                    const numericValue = parseFloat(valueString) * (cells[1].innerText.includes('Jt') ? 1000000 : (cells[1].innerText.includes('M') ? 1000000000 : 1));
        ...
        async function exportToPDF() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    a.href = url;
                    a.download = 'Sales_Recap_Report.pdf';
        ...
        /**
         * Sets up modal functionality (open/close).
         */
