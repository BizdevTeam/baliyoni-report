...
<head>
    ...
    <title>Rekap Penjualan instansi</title>
    ...
    <div class="wrapper">
        ...
        <div id="admincontent" class="content-wrapper ml-64 p-4 bg-gray-100 duration-300">
            <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold text-red-600 mb-2 font-montserrat">Rekap Penjualan instansi</h1>
                <!-- Button Tambah Data -->
                    ...
                    <div class="bg-white p-6 rounded shadow w-full max-w-md">
                        ...
                        <form id="modal-form" class="space-y-4" method="POST">
                            ...
                            <div id="instansi-container">
                                <div class="instansi-item flex items-center space-x-2 mb-2">
                                    <select name="instansi[]"
                                        class="w-full border-gray-300 rounded p-2 instansi-select" required>
                                    ...
                                    <input type="text" name="nilai[]"
                                        class="w-full border-gray-300 rounded p-2" placeholder="Nilai per Instansi" required>
                                    <button type="button"
                                        class="remove-instansi bg-red-600 text-white px-2 py-1 rounded">Hapus</button>
                                </div>
                            </div>
                            <button type="button" id="add-instansi"
                ...
                <table class="w-full table-auto border-collapse border border-gray-300 mt-6">
                    <thead class="bg-gray-200">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2">Nilai per Instansi</th>
        ...
        <script>
            document.getElementById('add-instansi').addEventListener('click', () => {
                const instansiContainer = document.getElementById('instansi-container');

                // Membuat elemen instansi dan Nilai per Instansi
                const newinstansiItem = document.createElement('div');
                newinstansiItem.className = 'instansi-item flex items-center space-x-2 mb-2';

                const instansiSelect = document.createElement('select');
                instansiSelect.name = 'instansi[]';
                instansiSelect.className = 'w-full border-gray-300 rounded p-2 instansi-select';
                instansiSelect.required = true;

                // Menambahkan opsi default dan instansi
                ...
                paketInput.placeholder = 'Nilai per Instansi';
                paketInput.required = true;

                const removeButton = document.createElement('button');
                removeButton.type = 'button';
                removeButton.className = 'remove-instansi bg-red-600 text-white px-2 py-1 rounded';
                removeButton.textContent = 'Hapus';

                // Menambahkan logika hapus
                removeButton.addEventListener('click', () => {
                    newinstansiItem.remove();
                });

                // Menambahkan elemen ke container
                newinstansiItem.appendChild(instansiSelect);
                newinstansiItem.appendChild(paketInput);
                newinstansiItem.appendChild(removeButton);

                instansiContainer.appendChild(newinstansiItem);
            });
            ...
            function isDuplicateEntry(bulanTahun, instansiList, items) {
                const hasDuplicate = new Set(instansiList).size !== instansiList.length;
                if (hasDuplicate) {
                    alert('instansi yang sama tidak boleh ditambahkan dalam bulan/tahun yang sama.');
                    return true;
                }

                return items.some(item => item.bulan_tahun === bulanTahun && instansiList.includes(item.instansi));
            }
            ...
            modalForm.addEventListener('submit', async (e) => {
                ...
                const instansiList = [...document.querySelectorAll('select[name="instansi[]"]')].map(select =>
                    select.value.trim());
                const nilaiPaketList = [...document.querySelectorAll('input[name="nilai[]"]')].map(
                    input => parseFloat(input.value.trim()));

                if (!bulanTahun || instansiList.some(p => !p) || nilaiPaketList.some(isNaN)) {
                ...
                const url = editMode ? `/marketings/laporanperinstansi/update/${editId}` :
                    '/marketings/laporanperinstansi/store';
            ...
            async function deleteData(id) {
                ...
                try {
                    const response = await fetch(`/marketings/laporanperinstansi/destroy/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        },
            ...
            async function updateData(filter = '') {
                const url = filter ? `/marketings/laporanperinstansi/data?bulan_tahun=${filter}` :
                    '/marketings/laporanperinstansi/data';
            ...
            function updateChart(items) {
                const labels = items.map((item) => item.instansi); // Label dari nama instansi
                const dataValues = items.map((item) => item.nilai); // Nilai per Instansi
                ...
                window.myChart = new Chart(ctx, {
                    ...
                    data: {
                        labels,
                        datasets: [{
                            label: 'GRAFIK NILAI BERDSARKAN INSTANSI',
                            data: dataValues,
            ...
            function editData(id, data) {
                ...
                // Pastikan `instansi` adalah array
                parsedData.instansi = Array.isArray(parsedData.instansi) ?
                    parsedData.instansi :
                    (typeof parsedData.instansi === 'string' ? parsedData.instansi.split(',') : []);
                ...
                // Bersihkan container instansi
                const instansiContainer = document.getElementById('instansi-container');
                instansiContainer.innerHTML = '';

                // Tambahkan elemen instansi dan Nilai per Instansi
                parsedData.instansi.forEach((instansi, index) => {
                    const newinstansiItem = document.createElement('div');
                    newinstansiItem.className = 'instansi-item flex items-center space-x-2 mb-2';
                    ...
                    nilaiPaketInput.placeholder = 'Nilai per Instansi';
                    nilaiPaketInput.value = parsedData.nilai[index] || ''; 
                    ...
                    newinstansiItem.appendChild(instansiSelect);
                    newinstansiItem.appendChild(nilaiPaketInput);
                    newinstansiItem.appendChild(removeButton);
