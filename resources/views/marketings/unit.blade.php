...
<head>
    ...
    <title>Add Unit Business Data</title>
...
<body class="bg-white hold-transition sidebar-mini layout-fixed">
    <div class="wrapper">
        ...
        <div id="admincontent" class="mt-14 content-wrapper ml-64 p-4 bg-white duration-300">
            <h1 class="flex text-4xl font-bold text-red-600 justify-center mt-4">Add Unit Business Data</h1>

            <div class="flex items-center justify-end transition-all duration-500 mt-8 mb-4">
                <!-- Search -->
                <form method="GET" action="{{ route('unit.index') }}" class="flex items-center gap-2">
            ...
            <div id="formContainer" class="visible">
                <div class="mx-auto bg-white p-6 rounded-lg shadow">
                        ...
                        <table class="table-auto w-full border-collapse border border-gray-300" id="data-table">
                            <thead class="bg-gray-200">
                                <tr>
                                    ...
                                    <th class="border border-gray-300 px-4 py-2 text-center">Unit Business Name</th>
                                    <th class="border border-gray-300 px-4 py-2 text-center">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($unit_bisnis as $nama_unit)
                                <tr class="hover:bg-gray-100">
                                    <td class="border border-gray-300 px-4 py-2 text-center">{{
                                        $nama_unit->id }}</td>
                                    <td class="border border-gray-300 px-4 py-2 text-center">{{
                                        $nama_unit->nama_unit }}</td>
                                    <td class="border border-gray-300 py-6 text-center flex justify-center gap-2">
                                        <!-- Edit Button -->
                                        <button class="text-red-500 px-3 py-2 rounded"
                                            data-modal-target="#editEventModal{{ $nama_unit->id }}">
                                            <i class="fa fa-pen"></i>
                                        </button>
                                        <!-- Delete Form -->
                                        <form method="POST"
                                            action="{{ route('unit.destroy', $nama_unit->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button class="text-red-500 px-3 py-2 rounded"
                                ...
                                <div class="fixed z-50 inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden"
                                    id="editEventModal{{ $nama_unit->id }}">
                                    <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                                        <h3 class="text-xl font-semibold mb-4">Edit Data</h3>
                                        <form method="POST"
                                            action="{{ route('unit.update', $nama_unit->id) }}"
                                            enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <div class="space-y-4">
                                                <div>
                                                    <label for="nama_unit" class="block text-sm font-medium">Unit Business Name</label>
                                                    <input type="number" name="nama_unit"
                                                        class="w-full p-2 border rounded"
                                                        value="{{ $nama_unit->nama_unit }}" required>
                                                </div>
                                            </div>
                    ...
                    <div class="m-4">
                        {{ $unit_bisnis->links('pagination::tailwind') }}
                    </div>
            ...
            <div class="bg-white w-1/2 p-6 rounded shadow-lg">
                ...
                <form method="POST" action="{{ route('unit.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="nama_unit" class="block text-sm font-medium">Unit Business Name</label>
                            <input type="text" name="nama_unit" class="w-full p-2 border rounded" required>
                        </div>
                    </div>
                    <div class="mt-4 flex justify-end gap-2">
                        <button type="button" class="bg-red-600 text-white px-4 py-2 rounded"
