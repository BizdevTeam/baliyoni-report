                    ...
                    <tbody>
                        @foreach ($laporans as $laporan)
                            <tr class="hover:bg-gray-100">
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->bulan_tahun }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->judul }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->aspek }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->masalah }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->solusi }}</td>
                                <td class="border border-gray-300 px-4 py-2 text-center">{{ $laporan->implementasi }}</td>
                            ...
                            <!-- Modal for Edit Data -->
                            <div class="fixed z-50 overflow-y-auto inset-0 bg-black bg-opacity-50  w-full flex items-center justify-center hidden" id="editEventModal{{ $laporan->id_spi }}">
                                    ...
                                    <form method="POST" action="{{ route('laporanspi.update', $laporan->id_spi) }}">
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                                ...
                                                <input type="month" name="bulan_tahun" class="w-full p-2 border rounded" value="{{ $laporan->bulan_tahun }}" required>
                                            </div>
                                            <div>
                                                <label for="judul" class="block text-sm font-medium">Judul</label>
                                                <textarea name="judul" class="w-full p-2 border rounded" rows="1" required>{{ $laporan->judul }}</textarea>
                                            </div>
                                            <div>
                                                <label for="aspek" class="block text-sm font-medium">Aspek</label>
                                                <textarea name="aspek" class="w-full p-2 border rounded" rows="1" required>{{ $laporan->aspek }}</textarea>
                                            </div>
                                            <div>
                                                <label for="masalah" class="block text-sm font-medium">Masalah</label>
                                                <textarea name="masalah" class="w-full p-2 border rounded" rows="1" required>{{ $laporan->masalah }}</textarea>
                                            </div>
                                            <div>
                                                <label for="solusi" class="block text-sm font-medium">Solusi</label>
                                                <textarea name="solusi" class="w-full p-2 border rounded" rows="1" required>{{ $laporan->solusi }}</textarea>
                                            </div>
                                            <div>
                                                <label for="implementasi" class="block text-sm font-medium">Implementasi</label>
                                                <textarea name="implementasi" class="w-full p-2 border rounded" rows="1" required>{{ $laporan->implementasi }}</textarea>
                                            </div>
                                        </div>
